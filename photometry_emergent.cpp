...
int main()
{
    ...
    double L0, Lprime, f0, f_r, f_i, delta_f, df_dr, xi_r, xi_i, c_x, u_x, R, r, omegat, theta_0, phi_0, C_1, phase, nprime_dot_n_obs, h_x, dA, dA0, dL, r_dot_n_obs, dL_f, dL_n, dL_h, dL0;
    ...
    outfile.open("Output/luminosities_emergent.dat", ios::out);
        ...
        for (alpha=0; alpha < N; alpha = alpha+1) {
            ...
            // Here is where the evaluation of the bits in the sum actually happens
                ...
                if (r_dot_n_obs > 0.0) {
                    ...
                    dL0 = f0 * r_dot_n_obs * (h_x/(2.0*pi)) * dA;
                    ...
                    dL = f0 * r_dot_n_obs * ( (c_x*u_x*nprime_dot_n_obs) / (2.0 * pi) ) * dA;
                    
                    dL_h = dL_h + dL;
                    
                    // change in flux
                    dL = delta_f * r_dot_n_obs * (h_x/(2.0*pi)) * dA;
                    
                    dL_f = dL_f + dL;
                    
                    // change in normal
                    dL = f0 * nprime_dot_n_obs * (h_x/(2.0*pi)) * dA;
